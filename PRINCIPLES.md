...
- **状态不是新颜色。** Live / Routing / Archived / Offline 走"活跃度梯度":复用 cyan(`--cs-state-live` / `--cs-state-routing`)与 ink 透明度(`--cs-state-archived` / `--cs-state-offline`),并叠加信号点形态(实心 / 脉冲 / 空心),让状态不只靠单一颜色通道。Live 与 Routing 同色,差异在**运动**(脉冲 vs 常亮),不在色相。**数据状态绝不用 amber**——amber 永远留给"当前激活 / 选中项"这种稀缺的交互态,否则预算一屏即爆、且与焦点态语义打架。
...
| 状态走语义梯度(cyan + 透明度 + 点的形态) | 给数据状态配红/绿语义色,或用 amber 标 live |
| 直角 | 随手加圆角 |
| 透明度拉层级 | 引入新颜色拉层级 |
